...
###################################### model setting #############################################################
DESCRIPTION = "convnextv2_tiny, data argumentation(RandAugment, cutout(nholes=1, size=8)), gradual unfreeze(epoch=5), smoothing=0.05 -> 0.1" # 예시: 실험 내용 기록용(한글 작성시 깨짐)

LOAD_WEIGHT = False # 기존 모델 가중치를 가져올지 여부
weight_save_path = './pretrained/save/0604_0053/0604_0053'
WEIGHT_PATH = "./pretrained/save/0604_0053/weight/best_weight.pth" # 기존 모델 가중치 경로
...
# 마지막 1~2개 스테이지만 학습
for param in model.stages[2].parameters(): # 1 epoch에 9시간
...
cfg = {
    'epoch': 50, # epoch 크기가 달라지면 CosineAnnealingLR 부분도 달라짐..!
    'lr': 0.0005,
    'weight_decay': 0.005
}
