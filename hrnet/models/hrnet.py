...
class HighResolutionNet(nn.Module):
    def __init__(self, cfg, num_classes=100, **kwargs):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False) # 기존 stride=2, branch 4의 feature map size가 1이 되는 상황을 방지.
        self.bn1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False) # 기존 stride=2, res = [8, 4]
    ...
    def _make_head(self, pre_stage_channels):
        ...
        head_channels = [32, 64, 128, 256] # head channels 크기 조정
        ...
        final_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=head_channels[3] * head_block.expansion, # head_channels의 마지막 원소 index(stage를 2까지로 축소했으니 1)
                out_channels=2048,
                kernel_size=1,
                stride=1,
                padding=0,
