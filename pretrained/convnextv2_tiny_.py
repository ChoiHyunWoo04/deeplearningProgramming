...
###################################### model setting #############################################################
DESCRIPTION = "convnextv2_tiny, data argumentation(RandAugment, mixup, cutout(nholes=1, size=56)), gradual unfreeze(epoch=7), smoothing=0.1" # 예시: 실험 내용 기록용(한글 작성시 깨짐)
    ...
    for epoch in range(cfg['epoch']):
        ...
        if epoch == 14:
            ...
            for name, module in model.get_stage(2).named_modules():
                if any(name.startswith(f'blocks.{i}') for i in [3, 4, 5, 6, 7, 8]):
