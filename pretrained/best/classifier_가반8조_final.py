...
if (LOAD_WEIGHT==False):  
    for epoch in range(cfg['epoch']):
        ...
        '''
        if epoch == 10: # 8만 열어도 너무 오래 걸림, 26까지 있는 것을 확인
            print("Unfreezing normalization layers in stage2 blocks 22-26 only")
            norm_params = []

            for name, module in model.get_stage(2).named_modules():
                if any(name.startswith(f'blocks.{i}') for i in [22, 23, 24, 25, 26]):
            ...
            if norm_params:
                optimizer.add_param_group({'params': norm_params, "lr": 5e-5})
        '''
        model.train()
